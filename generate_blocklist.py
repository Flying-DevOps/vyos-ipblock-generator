...
class BlocklistGenerator:
    ...
    def fetch_emerging_threats(self) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            self.logger.info(
                f"Emerging Threats: {len(ipv4_data)} IPv4, {len(ipv6_data)} IPv6 valid entries found"
            )
            return ipv4_data, ipv6_data
        except APIFetchError:
            self.logger.warning(
                "Failed to fetch Emerging Threats data, continuing without it"
            )
            return [], []
    ...
    def fetch_binary_defense(self) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            ipv4_data, ipv6_data = self._filter_lines(data)
            self.logger.info(
                f"Binary Defense: {len(ipv4_data)} IPv4, {len(ipv6_data)} IPv6 valid entries found"
            )
            return ipv4_data, ipv6_data
        except APIFetchError:
            self.logger.warning(
                "Failed to fetch Binary Defense data, continuing without it"
            )
            return [], []
    ...
    def fetch_abuseipdb(self) -> Tuple[List[str], List[str]]:
        ...
        params = {"confidenceMinimum": self.config.ABUSEIPDB_CONFIDENCE_MINIMUM}

        try:
            data = self._fetch_url(
                self.config.ABUSEIPDB_URL, headers=headers, params=params
            )
            ipv4_data, ipv6_data = self._filter_lines(data)
            self.logger.info(
                f"AbuseIPDB: {len(ipv4_data)} IPv4, {len(ipv6_data)} IPv6 valid entries found"
            )
            return ipv4_data, ipv6_data
        except APIFetchError:
    ...
    def _convert_to_cidr_v4(self, ip_list: List[str]) -> List[ipaddress.IPv4Network]:
        ...
        if conversion_errors > 0:
            self.logger.warning(
                f"Failed to convert {conversion_errors} IPv4 entries to CIDR"
            )

        self.logger.info(
            f"Successfully converted {len(cidr_list)} IPv4 entries to CIDR"
        )
        return cidr_list
    ...
    def _convert_to_cidr_v6(self, ip_list: List[str]) -> List[ipaddress.IPv6Network]:
        ...
        if conversion_errors > 0:
            self.logger.warning(
                f"Failed to convert {conversion_errors} IPv6 entries to CIDR"
            )

        self.logger.info(
            f"Successfully converted {len(cidr_list)} IPv6 entries to CIDR"
        )
        return cidr_list

    def _deduplicate_and_filter_list_v4(
        self, cidr_list: List[ipaddress.IPv4Network]
    ) -> List[ipaddress.IPv4Network]:
        """Remove duplicate and redundant IPv4 CIDR blocks."""
        self.logger.info(
            f"Deduplicating and filtering {len(cidr_list)} IPv4 CIDR entries"
        )
        ...
        self.logger.info(
            f"Removed {removed_count} redundant IPv4 entries, {len(filtered_list)} unique entries remaining"
        )
        return filtered_list

    def _deduplicate_and_filter_list_v6(
        self, cidr_list: List[ipaddress.IPv6Network]
    ) -> List[ipaddress.IPv6Network]:
        """Remove duplicate and redundant IPv6 CIDR blocks."""
        self.logger.info(
            f"Deduplicating and filtering {len(cidr_list)} IPv6 CIDR entries"
        )
        original_count = len(cidr_list)
        ...
        removed_count = original_count - len(filtered_list)
        self.logger.info(
            f"Removed {removed_count} redundant IPv6 entries, {len(filtered_list)} unique entries remaining"
        )
        return filtered_list
    ...
    def _apply_whitelist_filter_v4(
        self, cidr_list: List[ipaddress.IPv4Network]
    ) -> List[ipaddress.IPv4Network]:
        """Filter out whitelisted IPv4 networks from the blocklist."""
        if not self.whitelist_ipv4:
            self.logger.debug(
                "No IPv4 whitelist configured, skipping IPv4 whitelist filtering"
            )
            return cidr_list
        ...
        self.logger.info(
            f"IPv4 whitelist filtering: removed {whitelisted_count} entries, {len(filtered_list)} entries remaining"
        )
        return filtered_list

    def _apply_whitelist_filter_v6(
        self, cidr_list: List[ipaddress.IPv6Network]
    ) -> List[ipaddress.IPv6Network]:
        """Filter out whitelisted IPv6 networks from the blocklist."""
        if not self.whitelist_ipv6:
            self.logger.debug(
                "No IPv6 whitelist configured, skipping IPv6 whitelist filtering"
            )
            return cidr_list
        ...
        for network in cidr_list:
            if not isinstance(network, ipaddress.IPv6Network):
                self.logger.warning(
                    f"Non-IPv6Network found in IPv6 filter: {network!r}"
                )
    ...
    def generate_blocklist(
        ...
        self.logger.info(
            f"Total raw entries collected: {len(ipv4_blocklist)} IPv4, {len(ipv6_blocklist)} IPv6"
        )
        ...
        ipv4_cidr_list = (
            self._convert_to_cidr_v4(ipv4_blocklist) if ipv4_blocklist else []
        )
        ipv6_cidr_list = (
            self._convert_to_cidr_v6(ipv6_blocklist) if ipv6_blocklist else []
        )

        ipv4_deduplicated = (
            self._deduplicate_and_filter_list_v4(ipv4_cidr_list)
            if ipv4_cidr_list
            else []
        )
        ipv6_deduplicated = (
            self._deduplicate_and_filter_list_v6(ipv6_cidr_list)
            if ipv6_cidr_list
            else []
        )

        ipv4_filtered = (
            self._apply_whitelist_filter_v4(ipv4_deduplicated)
            if ipv4_deduplicated
            else []
        )
        ipv6_filtered = (
            self._apply_whitelist_filter_v6(ipv6_deduplicated)
            if ipv6_deduplicated
            else []
        )

        self.logger.info(
            f"Final blocklist contains {len(ipv4_filtered)} IPv4 and {len(ipv6_filtered)} IPv6 entries"
        )
        return ipv4_filtered, ipv6_filtered

    def _create_nft_rule_file_dual(
        self,
        ipv4_list: List[ipaddress.IPv4Network],
        ipv6_list: List[ipaddress.IPv6Network],
    ) -> str:
        """
        ...
        self.logger.info(
            f"Creating nftables rule file with {len(ipv4_list)} IPv4 and {len(ipv6_list)} IPv6 entries"
        )

        if self.dry_run:
            self.logger.info("DRY RUN: Would create nftables rule file")
            self.logger.info(
                f"DRY RUN: File would contain {len(ipv4_list)} IPv4 and {len(ipv6_list)} IPv6 CIDR entries"
            )
            if ipv4_list:
            ...
            return tempfile.mktemp(
                suffix="-dry-run-blocklist.nft"
            )  # nosec B306 - dry run only
...
"""
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, suffix=".nft"
        ) as temp_file:
    ...
    def _ensure_nft_set_exists_single(
        self, set_name: str, addr_type: str, table_family: str
    ) -> None:
    ...
    def _flush_nft_set_single(self, set_name: str, table_family: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully flushed {table_family} nftables set {set_name}"
            )
        except subprocess.CalledProcessError as e:
            error_msg = (
                f"Error flushing {table_family} nftables set {set_name}: {e.stderr}"
            )
            self.logger.error(error_msg)
            raise NFTConfigError(error_msg) from e
    ...
    def run(self) -> None:
        ...
        try:
            ...
            nft_filename = None
            try:
                nft_filename = self._create_nft_rule_file_dual(
                    ipv4_blocklist, ipv6_blocklist
                )
                ...
                if self.dry_run:
                    self.logger.info(
                        "DRY RUN: Blocklist generation completed successfully"
                    )
                    self.logger.info(
                        f"DRY RUN: Would have applied {len(ipv4_blocklist)} IPv4 and {len(ipv6_blocklist)} IPv6 entries"
                    )
                else:
                    self.logger.info("Blocklist successfully applied to nftables")
                    self.logger.info(
                        f"Applied {len(ipv4_blocklist)} IPv4 and {len(ipv6_blocklist)} IPv6 entries"
                    )
  ...
  %(prog)s --whitelist /path/to/whitelist.txt  # Use custom whitelist file
    ...
    try:
        generator = BlocklistGenerator(
            dry_run=args.dry_run, verbose=args.verbose, whitelist_file=args.whitelist
        )
        generator.run()
        return 0
